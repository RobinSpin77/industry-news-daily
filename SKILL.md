...
description: 通用的行业资讯日报/周报/月报生成器。按用户配置的行业领域、关键词和资讯来源，搜集最新动态，交叉验证后生成中文报道，推送到飞书群，并保存本地Markdown文件。支持日报（默认）、周报（周一或手动触发）、月报（每月25日或手动触发）。也支持自动发现行业权威媒体/信源。当用户提及"今日动态""行业日报""查一下最近XX新闻""XX行业有什么新消息""帮我搜集XX资讯""帮我找XX领域的权威媒体""XX行业有哪些信息源"及类似变体时触发。
---
