...
description: 通用的行业资讯日报/周报/月报生成器。按用户配置的行业领域、关键词和资讯来源，搜集最新动态，交叉验证后生成中文报道，推送到飞书群，并保存本地Markdown文件。支持日报（默认）、周报（周一或手动触发）、月报（每月25日或手动触发）。也支持自动发现行业权威媒体/信源、以及飞书机器人 Webhook 配置引导。当用户提及"今日动态""行业日报""查一下最近XX新闻""XX行业有什么新消息""帮我搜集XX资讯""帮我找XX领域的权威媒体""XX行业有哪些信息源""帮我配置飞书机器人""飞书推送""Webhook 地址"及类似变体时触发。
---
...
用户不需要手动罗列自己领域的媒体。当用户说「帮我在 config.json 中补充 XX 领域的信源」「帮我找 XX 行业的权威媒体」「XX 领域有哪些值得关注的信息来源」时，执行以下流程：

1. **搜索**：用 WebSearch 搜 `"{领域名} 行业媒体 权威网站 资讯来源"` 和 `"top {domain} industry news sites publications"` 各一轮
2. **筛选**：从搜索结果中提取 15-20 个候选来源，优先选取：
   - 该领域垂直专业媒体（如 MRAM → MRAM-Info）
   - 有影响力的行业门户（更新频率高、有原创报道）
   - 核心厂商的官网 news/press 页面
   - 覆盖投融资动态的财经媒体
   - 该领域的学术/科研进展追踪站点
3. **排名展示**：以表格呈现给用户，包含：来源名称、URL、类型（垂直媒体/综合科技/厂商官网/学术/投融资）、推荐理由（一句话）。标注推荐放入 priority 还是 normal
4. **用户确认**：用户勾选或说「全部采用」后，将选中的来源写入对应 category 的 `sources` 字段（或 `shared_sources`，根据来源的通用性判断）
5. **写入 config.json**：用 Edit 工具更新配置文件

如果用户尚未创建 config.json，先引导用户从 config.example.json 复制。

## 配置辅助：飞书机器人 Webhook

当用户说「帮我配置飞书机器人」「飞书推送怎么设置」或直接提供 Webhook URL 时：

1. **如果用户提供了 Webhook URL**：直接用 Edit 工具写入 `config.json` 的 `feishu_webhook` 字段，确认完成
2. **如果用户不知道如何获取**：给出去引导——​
   - 打开飞书 → 目标群聊 → 群设置（右上角 ···） → 群机器人 → 添加机器人 → 自定义机器人
   - 机器人名字建议填「行业资讯日报」
   - 安全设置按需配置（建议勾选「仅群主/特定成员可见」避免无关人员触发）
   - 复制生成的 Webhook 地址，发给 Claude 即可
